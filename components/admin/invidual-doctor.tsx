...
export function IndividualDoctorsManagement() {
  ...
  const [loading, setLoading] = useState<boolean>(false); // loader state
    ...
    const [openAddDoctor, setOpenAddDoctor] = useState(false);
      ...
      try {
        const res = await fetch(`${BASE_URL}doctor-profile/individual/doctor`
          ,{
            headers: {
  ...
  return (
    <div className="min-h-screen bg-white p-8">
      ...
      />
        ...
        {openAddDoctor && (
        <AddDoctorModal onClose={() => setOpenAddDoctor(false)} fetchDoctors={fetchDoctors} />
      )}
    </div>
  )
}
