...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      const response = await import('@/lib/Api/Auth/api').then(mod => mod.login(email, password))
      ...
      if (response.id && response.access_token) {
        ...
        // Fetch user profile
        const getUserProfile = await import('@/lib/Api/Auth/getUserProfile').then(mod => mod.getUserProfile);
        const profile = await getUserProfile(response.id, response.access_token);
      ...
      throw new Error('Login response missing user data. See console for details.');
    } catch (error) {
...
export function withRole<P extends object>(Component: React.ComponentType<P>, role: 'doctor' | 'patient') {
  return function RoleProtectedRoute(props: P) {
    ...
    return isAuthenticated && user?.role === role ? <Component {...props as P} /> : null
  }
}